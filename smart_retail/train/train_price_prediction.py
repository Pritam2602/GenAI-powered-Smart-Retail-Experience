...
class FastPricePredictor:
    ...
    def plot_metrics(self, product_type, y_true, y_pred, rmse, mae, r2, save_dir="artifacts/plots"):
        """Plot evaluation metrics and save them."""
        os.makedirs(f"{save_dir}/{product_type}", exist_ok=True)

        # --- Plot 1: Bar plot of metrics ---
        plt.figure(figsize=(8, 5))
        metrics = ["RMSE", "MAE", "R²"]
        values = [rmse, mae, r2]
        sns.barplot(x=metrics, y=values, palette="viridis")
        plt.title(f"{product_type.upper()} - Model Metrics")
        plt.ylabel("Score")
        plt.savefig(f"{save_dir}/{product_type}/metrics.png")
        plt.close()
        ...
        plt.title(f"{product_type.upper()} - Actual vs Predicted")
        plt.savefig(f"{save_dir}/{product_type}/actual_vs_pred.png")
        plt.close()
        ...
        plt.title(f"{product_type.upper()} - Error Distribution")
        plt.xlabel("Prediction Error")
        plt.ylabel("Count")
        plt.savefig(f"{save_dir}/{product_type}/error_dist.png")
        plt.close()

        print(f"📊 Saved plots for {product_type} under {save_dir}/{product_type}")
